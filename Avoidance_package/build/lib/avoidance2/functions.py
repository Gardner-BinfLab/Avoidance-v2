...
def csv_reader(file):
    ...
    '''
    df = pd.read_csv(file)
    if df.columns.str.contains('seq|sequence|sequences', regex=True).any():
        pass
    else:
...
def rand_background(seq, num=1000):
    '''Random background
    '''
    length = sequence_length(seq)
...
def syn_background(seq, n=1000):
    ...
    for j in range(n):
        ...
        for ind, codon in enumerate(codons):
            possible_codons = data.aa2codon[data.codon2aa[codon]]
...
def sequence_df_features(utr=None, positions=None, ncrna_file=None, \
                        length=None, input_df=None):
    ...
    input_df['cai'] = input_df['analyze'].apply(lambda x: \
                   x.cai())
    input_df['gc_cont'] = input_df['analyze'].\
                                apply(lambda x: x.gc_cont())
    input_df['sec_str'] = input_df['analyze'].\
                                apply(lambda x: x.sec_str(utr=utr, \
                                                positions=positions))
    input_df['avd'] = input_df['analyze'].\
                        apply(lambda x: x.avoidance(ncrna_file=\
                                                        ncrna_file))
    input_df['accs'] = input_df['analyze'].\
                                apply(lambda x: x.access_calc(\
                                            length=length, utr=utr))
    return input_df
